...
The 1.x series ran from `v1.0.0` (2026) through `v1.9.1` and was retired when
the project reset to the `0.y.z` release train. Its entries are kept here for
reference; see [CHANGELOG.md](../CHANGELOG.md) for current releases.

The GitHub release pages for these versions have been removed. The git tags
remain, so any version below can still be checked out and built from source:
...
- The unused `doc_draw` `box_iso` and `panel` special cases were removed. They
  ...
  operations. Dead Rust-only WCAG and area-downscale compatibility wrappers
...
- Compatibility-only input paths were removed. Document JSON and recipe
  ...
  longer accepts obsolete flags or the old macOS `launchctl load` fallback.
...
No entries were written for `v1.1.0`, `v1.0.1`, or `v1.0.0`. The tags remain in
the repository; use `git log v1.0.0..v1.1.0` to read what changed.
