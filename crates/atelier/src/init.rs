//! `atelier init` — stamp a directory-local store at `./.atelier/documents`.
//! Calls made from this directory then keep art and recipes here instead of in
//! the global store.
...
pub(crate) fn run(args: &[String]) -> i32 {
    ...
    match init_in(&cwd) {
        Ok((root, existed)) => {
            if existed {
                println!("already a local store: {}", root.display());
            } else {
                println!("local store created: {}", root.display());
            }
