...
6. **Compose, don't place every pixel.** Reach for the procedural/craft leverage
   ...
   `doc_batch` runs many ops on one cel in a single call.
...
Finish frame 0 as a clean pose first. Then per frame: `doc_add_frame copy_from`
the previous → repaint **only what moves** (`doc_pencil`, `doc_move_region`,
`doc_keyframe_move` for eased multi-frame motion) → `doc_render` (use `onion=true`
to see overlap). Tag the range with `doc_add_tag` (`forward`/`reverse`/`pingpong`).
Use `doc_tween` for dissolves, `doc_keyframe_move` arcs (ease-out rise, ease-in
fall) for jumps.
**Gates:** `doc_frame_diff` between adjacent frames — only the moving limbs
changed; `doc_anim_audit mode="spacing"` — motion even, low drift;
`doc_anim_audit mode="seam"` on the tag — loop wrap is clean (pingpong has no seam).
...
doc_create → make_perceptual_ramp/harmony_palette (lock) → block silhouette
   ...
   → detail + doc_batch → doc_relight (key/fill/rim) / doc_material / doc_dither_ramp
   ...
   → [animate: add_frame copy_from → repaint diff (transform_cel to move a part)
      → frame_diff/anim_audit (seam·spacing·arc) → contact_sheet → add_tag]
